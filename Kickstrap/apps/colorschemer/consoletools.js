...
function get(outputType) {
	if(!cColor) {
		...
		switch(outputType) {
			case 'rgb':
			Color = tinycolor(cColor).toRgbString();
			break;
			
			case 'hex':
			Color = tinycolor(cColor).toHexString();
			break;	
				
			case 'hsv':
			Color = tinycolor(cColor).toHsvString();
			break;
			
			case 'filter':
			Color = tinycolor(cColor).toFilter();
			break;
...
function churnColor(outputType, Colors) {
  ...
  for (var i = 0; i < Colors.length; i++) {
		if(outputType) {
			switch(outputType) {
			  case 'rgb':
			  colorOutput = Colors[i].toRgbString();
			  break;
			
			  case 'hsv':
			  colorOutput = Colors[i].toHsvString();
			  break;
			  
			  case 'hex':
			  colorOutput = Colors[i].toHexString();
			  break;
			  
			  case 'filter':
			  colorOutput = Colors[i].toFilter();
			  break;
